...
public class MobilogramSmootherTask extends AbstractTask {
  ...
  public void run() {
    ...
    for (Frame frame : frames) {
      List<IMobilogram> smoothedMobilograms = new ArrayList<>(frame.getMobilograms().size());

      for (IMobilogram mobilogram : frame.getMobilograms()) {

        if (isCanceled()) {
          return;
        }

        List<MobilityDataPoint> dataPoints = new ArrayList<>(mobilogram.getDataPoints());
