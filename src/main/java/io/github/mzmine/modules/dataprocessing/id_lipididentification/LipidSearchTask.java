...
public class LipidSearchTask extends AbstractTask {
	...
	public LipidSearchTask(ParameterSet parameters, PeakList peakList) {
		...
		this.minChainLength = parameters.getParameter(LipidSearchParameters.chainLength).getValue().lowerEndpoint();
		this.maxChainLength = parameters.getParameter(LipidSearchParameters.chainLength).getValue().upperEndpoint();
		this.minDoubleBonds = parameters.getParameter(LipidSearchParameters.doubleBonds).getValue().lowerEndpoint();
		this.maxDoubleBonds = parameters.getParameter(LipidSearchParameters.doubleBonds).getValue().upperEndpoint();
		...
		this.ionizationType = parameters.getParameter(LipidSearchParameters.ionizationMethod).getValue();
		this.searchForMSMSFragments = parameters.getParameter(LipidSearchParameters.searchForMSMSFragments).getValue();
		this.searchForModifications = parameters.getParameter(LipidSearchParameters.searchForModifications).getValue();
		...
		if (searchForMSMSFragments) {
			this.mzToleranceMS2 = parameters.getParameter(LipidSearchParameters.searchForMSMSFragments)
					.getEmbeddedParameters().getParameter(LipidSearchMSMSParameters.mzToleranceMS2).getValue();
			this.massListName = parameters.getParameter(LipidSearchParameters.searchForMSMSFragments)
					.getEmbeddedParameters().getParameter(LipidSearchMSMSParameters.massList).getValue();
			this.ionizationAutoSearch = parameters.getParameter(LipidSearchParameters.searchForMSMSFragments)
					.getEmbeddedParameters().getParameter(LipidSearchMSMSParameters.ionizationAutoSearch).getValue();
			this.keepUnconfirmedAnnotations = parameters.getParameter(LipidSearchParameters.searchForMSMSFragments)
					.getEmbeddedParameters().getParameter(LipidSearchMSMSParameters.keepUnconfirmedAnnotations)
					.getValue();
		} else {
		...
		if (searchForCustomLipidClasses) {
			this.customLipidClasses = LipidSearchParameters.customLipidClasses.getEmbeddedParameter().getChoices();
		}
		...
		for (int i = 0; i < selectedLipids.length; i++) {
			...
			for (int chainLength = minChainLength; chainLength <= maxChainLength; chainLength++) {
				for (int chainDoubleBonds = minDoubleBonds; chainDoubleBonds <= maxDoubleBonds; chainDoubleBonds++) {
					...
					lipidDatabase.add(new LipidIdentity(selectedLipids[i], chainLength, chainDoubleBonds, chainTypes,
							LipidClassType.LIPID_CLASS));
				}
			}
		}
	...
	private void findPossibleLipid(LipidIdentity lipid, PeakListRow row) {
		...
		for (IonizationType ionization : ionizationTypeList) {
			if (!row.getBestPeak().getRepresentativeScan().getPolarity().equals(ionization.getPolarity())) {
				continue;
			}
			...
			if (mzTolRange12C.contains(lipidIonMass)) {
				...
				row.setComment("Ionization: " + ionization.getAdduct() + ", Δ "
				...
				logger.info("Found lipid: " + lipid.getName() + ", Δ " + NumberFormat.getInstance().format(relMassDev)
						+ " ppm");
			}
	...
	/**
	 * This method searches for MS/MS fragments. A mass list for MS2 scans will be
	 * used if present. If no mass list is present for MS2 scans it will create one
	 * using centroid or exact mass detection algorithm
	 */
	private void searchMsmsFragments(PeakListRow row, IonizationType ionization, LipidIdentity lipid) {

		// Check if selected feature has MSMS spectra and LipidIdentity
		if (row.getAllMS2Fragmentations().length > 0 && row.getPreferredPeakIdentity() instanceof LipidIdentity) {
			...
			if (rules.length > 0) {
				for (int j = 0; j < massList.length; j++) {
					...
					LipidFragment annotatedFragment = msmsLipidTools.checkForClassSpecificFragment(mzTolRangeMSMS,
							(LipidIdentity) row.getPreferredPeakIdentity(), ionization, rules, massList[j].getMZ());
			...
			if (!listOfAnnotatedFragments.isEmpty()) {
				...
				confirmFormula = msmsLipidTools.confirmHeadgroupFragmentPresent(listOfAnnotatedFragments);

				// predict lipid fatty acid composition if possible
				if (lipid.getLipidClassType().equals(LipidClassType.LIPID_CLASS)) {
					listOfChainCompositions = msmsLipidTools.predictChainComposition(listOfAnnotatedFragments,
							row.getPreferredPeakIdentity(), lipid.getLipidClass().getChainTypes());
				} else if (lipid.getLipidClassType().equals(LipidClassType.CUSTOM_LIPID_CLASS)) {
					listOfChainCompositions = msmsLipidTools.predictChainComposition(listOfAnnotatedFragments,
							row.getPreferredPeakIdentity(), lipid.getCustomLipidClass().getChainTypes());
				}
		...
		if (listOfChainCompositions.isEmpty() && confirmFormula == false && keepUnconfirmedAnnotations == false) {
			row.removePeakIdentity(row.getPreferredPeakIdentity());
		}
		for (int j = 0; j < listOfChainCompositions.size(); j++) {
			String name = "";
			if (lipid.getLipidClassType().equals(LipidClassType.LIPID_CLASS)) {
				name = lipid.getLipidClass().getName() + " " + lipid.getLipidClass().getAbbr() + "("
						+ listOfChainCompositions.get(j).getName() + ")";
			} else if (lipid.getLipidClassType().equals(LipidClassType.CUSTOM_LIPID_CLASS)) {
				name = lipid.getCustomLipidClass().getName() + " " + lipid.getCustomLipidClass().getAbbr() + "("
						+ listOfChainCompositions.get(j).getName() + ")";
			}
			row.addPeakIdentity(new LipidIdentity(name, listOfChainCompositions.get(j).getFormula()), true);
			row.setPreferredPeakIdentity(new LipidIdentity(name, listOfChainCompositions.get(j).getFormula()));
		}
	}
}
...
private void searchModifications(PeakListRow rows, double lipidIonMass, LipidIdentity lipid,
	...
	for (int j = 0; j < lipidModificationMasses.length; j++) {
		if (mzTolModification.contains(lipidIonMass + (lipidModificationMasses[j]))) {
			...
			rows.setComment(rows.getComment() + " Ionization: " + ionizationType.getAdduct()
					+ " Warning: Lipid Annotation was not confirmed by MS/MS and needs to be checked manually!"
					+ lipidModification[j] + ", Δ " + NumberFormat.getInstance().format(relMassDev) + " ppm");
			logger.info(
